...
class GrpcClient: NSObject {
    ...
    func send(dataString: String) -> StreamingVoice_VoiceRequest? {
            guard let decodeStringBytesAudio = dataString.data(using: .utf8)?.base64EncodedString()
                    , let audioData = Data(base64Encoded: decodeStringBytesAudio, options: .ignoreUnknownCharacters) else {
                return nil
            }
            print(decodeStringBytesAudio)
            var voice = StreamingVoice_VoiceRequest()
            voice.byteBuff = audioData
            return voice
        }
